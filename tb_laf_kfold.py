...
def main() -> None:
    ...
    if args.k_folds < 2:
        raise ValueError(f"--k-folds must be at least 2, got {args.k_folds}")
    ...
    targets = _dataset_targets(train_ds)
    min_class_count = min(targets.count(class_index) for class_index in range(len(classes)))
    if args.k_folds > min_class_count:
        raise ValueError(
            f"--k-folds ({args.k_folds}) cannot exceed the smallest class count ({min_class_count})"
        )
    ...
    print(f"Total cross-validation samples: {len(train_ds)}")
    ...
    for model_name in args.models:
        model_fold_results = []
        for fold_index, (fold_train_indices, fold_val_indices) in enumerate(
            splitter.split(list(range(len(train_ds))), targets), start=1
        ):
            print(f"\n[{model_name}] Fold {fold_index}/{args.k_folds}")
            fold_train_ds = Subset(train_ds, fold_train_indices.tolist())
            fold_val_ds = Subset(train_ds, fold_val_indices.tolist())
            class_weights = compute_class_weights(fold_train_ds, num_classes=len(classes))
            print(f"[{model_name}] Fold {fold_index} class weights: {class_weights.tolist()}")

            batch_size = args.batch_size
            train_loader, val_loader, test_loader = build_loaders(
                fold_train_ds, fold_val_ds, test_ds, batch_size, args.num_workers
            )
        ...
        mean_best_val_f1 = sum(item["best_val_f1"] for item in model_fold_results) / args.k_folds
        mean_test_metrics = {}
        for metric_name in ("loss", "accuracy", "precision", "recall", "f1", "auroc"):
            metric_values = [
                item.get("test_metrics", {}).get(metric_name, 0.0)
                for item in model_fold_results
            ]
            mean_test_metrics[metric_name] = sum(metric_values) / args.k_folds

        results.append({
            "model": model_name,
            "k_folds": args.k_folds,
            "mean_best_val_f1": mean_best_val_f1,
            "mean_test_metrics": mean_test_metrics,
            "fold_results": model_fold_results,
        })

    results.sort(key=lambda item: item["mean_best_val_f1"], reverse=True)
    ...
    summary_payload = {
        ...
        "original_val_samples": len(val_ds),
    ...
    print("\nFinal ranking by mean best validation F1:")
    for rank, item in enumerate(results, start=1):
        print(f"{rank}. {item['model']}: mean_best_val_f1={item['mean_best_val_f1']:.4f}")
        test_metrics = item.get("mean_test_metrics") or {}
        print(
            f"   mean_test_f1={test_metrics.get('f1', 0.0):.4f} "
            f"mean_test_acc={test_metrics.get('accuracy', 0.0):.4f} "
            f"mean_test_precision={test_metrics.get('precision', 0.0):.4f} "
            f"mean_test_recall={test_metrics.get('recall', 0.0):.4f} "
            f"mean_test_auroc={test_metrics.get('auroc', 0.0):.4f}"
        )
        for fold_result in item["fold_results"]:
            print(
                f"   fold={fold_result['fold']} "
                f"best_val_f1={fold_result['best_val_f1']:.4f} "
                f"checkpoint={fold_result['best_checkpoint']}"
            )
